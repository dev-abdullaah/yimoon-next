...
const Featured = () => {
    ...
    // Get featured ID from URL parameters
    const featuredId = searchParams.get('featuredId');
    const featuredName = featuredId ? FEATURED_MAPPING[featuredId] || 'Featured Products' : 'Featured Products';
    ...
    // Invalidate queries when featuredId changes
    useEffect(() => {
        queryClient.invalidateQueries(['featuredProducts']);
    }, [featuredId, queryClient]);

    // Fetch product data
    const { data: catalogData, isLoading, isError } = useQuery({
        queryKey: ['featuredProducts', { featuredId, page: currentPage, sortBy }],
        queryFn: async () => {
            ...
            // Only add condition if featuredId exists
            if (featuredId) {
                formData.append('condition', `featured LIKE '${featuredId}%'`);
            }
